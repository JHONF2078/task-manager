...
class MailTransportInfoCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output) : int
    {
        ...
        foreach (['transport','transports'] as $propName) {
            if ($refMailer->hasProperty($propName)) {
                $transportProp = $refMailer->getProperty($propName);
                break;
            }
        }
        if ($transportProp) {
    ...
    {
        if (!$transport) {
            $io->warning("Transporte $label es null");
            return;
        }
        ...
        if (method_exists($transport, '__toString')) {
            try {
                $io->text('DSN (toString): ' . (string)$transport);
        ...
        foreach (['getHost','getPort','getEncryption'] as $m) {
            if (method_exists($transport, $m)) {
                try {
                    $io->text($m . ': ' . var_export($transport->$m(), true));
                } catch (\Throwable $e) {
                }
            }
        }
