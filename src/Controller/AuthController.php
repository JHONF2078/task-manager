...
class AuthController extends AbstractController
{
    ...
    public function register(Request $request): JsonResponse
    {
        ...
        if (count($violations) > 0) {
            $errors = [];
            foreach ($violations as $v) { $errors[] = ['field'=>$v->getPropertyPath(),'message'=>$v->getMessage()]; }
        ...
        try {
            ...
            if (!$user) { throw new ConflictException('El email ya está registrado'); }
    ...
    public function logout(Request $request): JsonResponse
    {
        $plainRefresh = $request->cookies->get('refresh_token');
        if ($plainRefresh) {
            try { $this->refreshTokenService->revokeChain($plainRefresh); } catch (\Throwable $e) { /* ignore */ }
        }
