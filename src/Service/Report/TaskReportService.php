...
class TaskReportService
{
    ...
    {
        $filters = [];
        if (!empty($criteria['status'])) $filters['status'] = $criteria['status'];
        if (!empty($criteria['priority'])) $filters['priority'] = $criteria['priority'];
        if (!empty($criteria['assignedTo'])) $filters['assignedTo'] = $criteria['assignedTo'];
        if (!empty($criteria['from'])) $filters['dueFrom'] = $criteria['from'];
        if (!empty($criteria['to'])) $filters['dueTo'] = $criteria['to'];
    ...
    public function summarize(array $tasks): array
    {
        ...
        ksort($byStatus); ksort($byPriority);
    ...
    public function toCsv(array $tasks, array $summary): string
    {
        ...
        foreach ($summary['byStatus'] as $k => $v) { $write([$k, $v]); }
        $write([]);
        $write(['Resumen por prioridad']);
        foreach ($summary['byPriority'] as $k => $v) { $write([$k, $v]); }
